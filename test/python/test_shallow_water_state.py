...
def test_shallow_water_state_init_optional(input_geometry, input_gt4py_config):
    ...
    assert_array_equal(state.u[xps-xms:xpe-xms+1, yps-yms:ype-yms+1], u)
    assert_array_equal(state.v[xps-xms:xpe-xms+1, yps-yms:ype-yms+1], v)
    assert_array_equal(state.h[xps-xms:xpe-xms+1, yps-yms:ype-yms+1], h)
...
def test_shallow_water_state_halo(input_geometry, input_gt4py_config):
    ...
    if (north != -1):
        assert_array_equal(state.u[xps-xms:xpe-xms+1, yme-yms], 10.0 * north)
        assert_array_equal(state.v[xps-xms:xpe-xms+1, yme-yms], 20.0 * north)
        assert_array_equal(state.h[xps-xms:xpe-xms+1, yme-yms], 30.0 * north)
    if (south != -1):
        assert_array_equal(state.u[xps-xms:xpe-xms+1, yms-yms], 10.0 * south)
        assert_array_equal(state.v[xps-xms:xpe-xms+1, yms-yms], 20.0 * south)
        assert_array_equal(state.h[xps-xms:xpe-xms+1, yms-yms], 30.0 * south)
    if (west != -1):
        assert_array_equal(state.u[xms-xms, yps-yms:ype-yms+1],  10.0 * west)
        assert_array_equal(state.v[xms-xms, yps-yms:ype-yms+1],  20.0 * west)
        assert_array_equal(state.h[xms-xms, yps-yms:ype-yms+1],  30.0 * west)
    if (east != -1):
        assert_array_equal(state.u[xme-xms, yps-yms:ype-yms+1], 10.0 * east)
        assert_array_equal(state.v[xme-xms, yps-yms:ype-yms+1], 20.0 * east)
        assert_array_equal(state.h[xme-xms, yps-yms:ype-yms+1], 30.0 * east)
...
def test_shallow_water_state_write(input_io_geometry, input_gt4py_config):
    ...
    state_read.read("../test_output/test_shallow_water_writer.nc")

    assert_almost_equal(state.clock, state_read.clock, decimal=9)

    assert_array_equal(state_read.u[xps-xms:xpe-xms+1, yps-yms:ype-yms+1], u)
    assert_array_equal(state_read.v[xps-xms:xpe-xms+1, yps-yms:ype-yms+1], v)
    assert_array_equal(state_read.h[xps-xms:xpe-xms+1, yps-yms:ype-yms+1], h)
